...
trait ParsesPlayers {
    ...
    public function parseMember($original, $guild, $logPrefix = '   ') {
        ...
        Member::withoutSyncingToSearch(function() use ($member, $guild) {
            if (!is_null($guild)) {
        ...
        Member::withoutSyncingToSearch(function() use ($member) {
            $member->save();
        });
        ...
        // $this->comment("${logPrefix}{$member->player} updating stat counts.");

        // $memberStats = $this->getMemberStats($member);

        // $stats = $member->stats;
        // $stats->data = collect($memberStats->first());
        // $stats->save();
