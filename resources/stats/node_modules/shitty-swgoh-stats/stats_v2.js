...
class Stats {
    ...
    getCharRawStats ( char ) {
        ...
        if (relicTier > 2) {
            ...
            const relic = relicData[ relicId ] || {stats: {}, gms: {}}
    ...
    getCrewRating ( crew ) {
        ...
        return crew.reduce((crewRating, char) => {
            ...
            var mr = char.equippedStatModList.reduce((cr, mod) => {
                if( !mod.pips ) {
                    var [ , pips, ] = mod.definitionId.split('')
                    mod.pips = pips
                }
    ...
    calculateModStats ( baseStats, mods ) {
        ...
        return Object.keys(rawModStats).reduce((modStats, statId) => {
            ...
            if( !statId || value < 0 ) return modStats
            switch ( statId ) {
                case 41: // Offense
                ...
                case 54: // Crit Avoid
                    value *= 1e-8
                    modStats[35] = (modStats[35] || 0) + (value * 100 * 1e8) // Ph. Crit Avoid
                    modStats[36] = (modStats[36] || 0) + (value * 100 * 1e8) // Sp. Crit Avoid
                    break
                case 55: // Heatlth %
                    value *= 1e-8
    ...
    formatStats ( stats, level ) {
        ...
        convertPercent(39, ( val ) => convertFlatCritAvoidToPercent( val, scale * descale * 100 ) ); // Physical Crit Avoidance
        convertPercent(40, ( val ) => convertFlatCritAvoidToPercent( val, scale * descale * 100 ) ); // Special Crit Avoidance
        ...
        if( stats.gear ) { // is Char
            ...
            statList.forEach( statId => {
                ...
                if( language.pct[Number(statId)] ) {
                    if( [21, 22].includes(Number(statId)) ) {
                        gsStats.final[ flatStatId ] = Math.floor(gsStats.final[ flatStatId ] * 100) / 100
                        gsStats.mods[ flatStatId ]  = Math.floor((gsStats.final[ flatStatId ] - (stats.base[flatStatId] * 100)) * 100) / 100
                    } else if( [35, 36].includes(Number(statId)) ) {
                        let base = Math.floor(stats.base[ flatStatId ] * 10000) / 100
                        gsStats.final[ statId ] = Math.floor(gsStats.final[ flatStatId ] * 100) / 100 + base
                        gsStats.mods[ statId ]  = gsStats.final[ statId ] - base
                    } else {
